...
function scheduleDates(business = {}, formalExamTime = "") {
  ...
  const matches = [...text(business.formal_exam_time_range || formalExamTime).matchAll(/\d{4}[/-]\d{1,2}[/-]\d{1,2}/g)]
...
export function buildContentRequirementEmail({ task = {}, requirement = {} } = {}) {
  ...
  const projectName = firstValue(latest.examName, latest.projectName, business.project_name, task.projectName);
  ...
  const trialExamTime = firstValue(latest.trialExamTime, latest.mockStartTimeDisplay, business.trial_exam_time);

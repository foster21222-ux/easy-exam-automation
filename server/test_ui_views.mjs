...
test("requirement center renders list and detail surfaces", () => {
  ...
  assert.ok(html.includes("formatRequirementChangeSummary"));
  assert.ok(html.includes("Array.from(requirementChangeFields(item))"));
...
test("WeChat configuration lives in project management and monitoring lives in system config", () => {
  ...
  assert.equal(html.includes('id="addWechatGroupBtn"'), false);
  ...
  assert.ok(html.includes("renderWechatRunConfirmationActions"));
  assert.ok(html.includes("taskViewState.wechatPendingConfirmation"));
  assert.ok(html.includes("/api/wechat-collector/user-confirmation/action"));
  assert.ok(html.includes("等待用户确认"));
  assert.ok(html.includes("本次等待"));
  assert.ok(html.includes("执行本项目群"));
  assert.ok(html.includes("执行全部等待群"));
  assert.ok(html.includes("data-wechat-group-name"));
  assert.equal(html.includes('groups.length > 1 ? `<button class="btn" data-wechat-user-confirmation-action="run-now" type="button">执行全部等待群</button>` : ""'), false);
  assert.ok(html.includes("15 分钟后提醒"));
  assert.ok(html.includes("微信群配置"));
  assert.ok(html.includes("本项目微信群"));
  assert.ok(html.includes("保存只影响当前项目群"));
  ...
  assert.ok(html.includes("全局兼容入口可留空；项目页会自动绑定项目已生成的配置需求单。"));
  ...
  assert.ok(html.includes('data-wechat-row-state'));
  assert.ok(html.includes(".wechat-row-state.success"));
  assert.ok(html.includes(".wechat-row-state.error"));
  assert.ok(html.includes("setWechatCollectorRowState(button"));
  ...
  assert.ok(html.includes("运营控制台自动化环境"));
  assert.ok(html.includes("检查运控自动化环境"));
  assert.ok(html.includes("安装浏览器自动化依赖"));
  assert.ok(html.includes("启用自动化并重启服务"));
  assert.ok(html.includes('id="operationConsoleEnvironment"'));
  assert.ok(html.includes("renderOperationConsoleEnvironment"));
  assert.ok(html.includes("/api/operation-console/environment"));
  assert.ok(html.includes("/api/operation-console/environment/install"));
  assert.ok(html.includes("/api/operation-console/environment/enable"));
  assert.ok(html.includes('id="emailSettingsPanel"'));
  assert.ok(html.includes("Outlook 公司邮箱"));
  assert.ok(html.includes('id="emailFromAddress"'));
  assert.ok(html.includes('id="emailPassword"'));
  assert.ok(html.includes('id="emailTestRecipient"'));
  assert.ok(html.includes('id="saveEmailSettingsBtn"'));
  assert.ok(html.includes('id="sendEmailTestBtn"'));
  assert.equal(html.includes("默认收件人"), false);
  assert.equal(html.includes("打开运控登录浏览器"), false);
  assert.equal(html.includes("登录态"), false);
...
test("project detail exposes initial requirement and project scoped WeChat entry", () => {
  ...
  assert.equal(html.includes('id="projectWechatConfigBtn"'), false);
  ...
  assert.ok(html.includes("renderProjectRequirementEditPanel"));
  assert.ok(html.includes("handleProjectRequirementStaffEdit"));
  assert.ok(html.includes("renderProjectRequirementChangeLog(detail, { actions: true })"));
  assert.ok(html.includes("project-requirement-change-list"));
  assert.ok(html.includes("formatRequirementChangeSummary(item)"));
  ...
  assert.ok(html.includes("人工修订配置需求"));
  assert.ok(html.includes("requirement_staff_edited"));
  assert.ok(html.includes("overrideManualEdit"));
  assert.ok(html.includes('const conflicts = pending && requirement ? manualEditConflictsForChange(requirement, item) : [];'));
  ...
  assert.equal(inlineRequirementSource.includes("版本历史"), false);
  assert.equal(inlineRequirementSource.includes("versionHtml"), false);
  ...
  assert.equal(html.includes("配置本项目微信群"), false);
  assert.ok(html.includes("运控协同"));
  assert.ok(html.includes('id="projectOperationBatchPanel"'));
  assert.ok(html.includes('id="projectOperationBatchDraft"'));
  assert.ok(html.includes('id="operationBatchCodeInput"'));
  assert.ok(html.indexOf('id="projectBusinessRequirement"') < html.indexOf('id="projectOperationBatchPanel"'));
  assert.ok(html.indexOf('id="projectOperationBatchPanel"') < html.indexOf('id="projectRequirementInlinePanel"'));
  assert.ok(html.includes("批次代码"));
  assert.equal(html.includes("业务编号"), false);
  assert.equal(html.includes("业务流转编号"), false);
  assert.equal(html.includes("保存批次草稿"), false);
  assert.equal(html.includes("operationBatchSaveDraftBtn"), false);
  assert.ok(html.includes("创建参数"));
  assert.ok(html.includes("创建未发布批次"));
  assert.ok(html.includes("手动补录批次代码"));
  assert.ok(html.includes("发送内容任务单"));
  assert.ok(html.includes('id="contentRequirementEmailRecipients"'));
  assert.ok(html.includes('id="contentRequirementEmailSendBtn"'));
  assert.ok(html.includes('id="contentRequirementEmailState"'));
  assert.ok(html.includes("/content-requirement-email"));
  assert.ok(html.includes("发送人员任务单"));
  assert.ok(html.includes("项目归档"));
  assert.ok(html.includes("function updateOperationBatchActions"));
  assert.ok(html.includes("operationBatchCreateBtn.disabled = Boolean(code);"));
  assert.ok(html.includes("operationBatchRecordBtn.disabled = Boolean(code);"));
  assert.ok(html.includes("/operation-batch/draft"));
  assert.ok(html.includes("/operation-batch/create"));
  assert.ok(html.includes("/operation-batch/result"));
  assert.ok(html.includes("operationBatchDisplayId"));
  assert.equal(html.includes("后续接入"), false);
  assert.equal(html.includes("暂未开放"), false);
  assert.ok(html.includes("projectWechatGroupDraft"));
  assert.ok(html.includes("task_id: identity.taskId"));
  assert.ok(html.includes('data-wechat-field="task_id"'));
  assert.ok(html.includes("wechat-project-form"));
  assert.ok(html.includes("wechat-project-status-grid"));
  assert.ok(html.includes(".wechat-project-status-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 14px; }"));
  assert.ok(html.includes("wechat-row-state"));
  assert.ok(html.includes("renderProjectWechatCollectorConfig"));
  assert.ok(html.includes("renderProjectWechatRequirementUpdates"));
  assert.ok(html.includes("renderProjectWechatRunTimeline"));
  assert.ok(html.includes("btn compact"));
  assert.ok(html.includes(">本群<"));
  assert.ok(html.includes(">全部<"));
  assert.ok(html.includes(">15分后<"));
  ...
  assert.ok(html.includes("已带入本项目微信群，请填写群名称后保存设置"));
  assert.ok(html.includes("bindProjectWechatGroup"));
  assert.equal(html.includes("项目名称、客户名称和需求单号由项目配置自动绑定"), false);
  assert.equal(html.includes("一个项目只关联一个微信群"), false);
});
...
test("project configuration owns requirement and WeChat configuration entry points", () => {
  ...
  assert.equal(html.includes('id="projectConfigurationReadiness"'), false);
  assert.ok(html.includes('id="projectConfigurationStatusChip"'));
  assert.ok(html.includes('id="projectWechatConfigPanel"'));
  assert.ok(html.includes('<details id="projectRequirementInlinePanel" open>'));
  assert.ok(html.includes('<details id="projectWechatConfigPanel" class="project-wechat-section" open>'));
  assert.equal(html.includes('<section id="projectWechatConfigPanel" class="project-wechat-section">'), false);
  assert.equal(html.includes('id="projectRequirementWorkQueue"'), false);
  assert.equal(html.includes("配置需求处理工作台"), false);
  assert.equal(html.includes("项目配置总览"), false);
  assert.ok(html.includes("renderProjectConfigurationStatus"));
  ...
  assert.ok(html.includes("validateProjectWechatSavePayload"));
  assert.ok(html.includes("当前项目身份不完整，不能保存本项目微信群"));
  assert.ok(html.includes("请先填写本项目微信群名称"));
  assert.ok(html.includes("微信群配置缺少项目绑定信息"));
